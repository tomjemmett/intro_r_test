...
<img src="img/session04/ggplot2_masterpiece.jpg" width="40%"/>
...
</br> 3. Attractive graphics (with a little
...
.footnote[
1. [This Photo](https://www.deviantart.com/luckymarine577/art/Animated-Ambulance-338970039) by Unknown Author is licensed under [CC BY SA NC](https://creativecommons.org/licenses/by-nc-sa/3.0/)
2. [This Photo](https://creativecommons.org/licenses/by-nc-sa/3.0/) by Unknown Author is licensed under [CC BY SA NC](https://creativecommons.org/licenses/by-nc-sa/3.0/)
]
...
The dataset we loaded earlier, .green[capacity_ae], shows </br>
...
<img class="center" src="img/session04/ggplot2_exploratory.jpg" width="50%">
...
.panel[.panel-name[Let's explain the code]
...
```{r paper-exercise, eval=TRUE, echo=FALSE}
...
# .black[`r icon::icon_style(fontawesome("square-full"), scale = 2)`]
...
# .black[`r icon::icon_style(fontawesome("circle", style = "solid"), scale = 2)`]
...
# .black[`r icon::icon_style(fontawesome("circle", style = "solid"))`]
...
# `r icon::icon_style(fontawesome("arrows-alt-h"), scale = 2)` .black[`r icon::icon_style(fontawesome("circle", style = "solid"), scale = 2)`]
...
# `r icon::icon_style(fontawesome("arrows-alt-v"), scale = 2)` .black[`r icon::icon_style(fontawesome("circle", style = "solid"), scale = 2)`]
...
# `r icon::icon_style(fontawesome("circle", style = "solid"), scale = 2)`
...
Data variables `r icon::fontawesome("arrow-right")` .blue[**geom**]etric objects
...
.blue[Here, other aesthetic properties (size, colour, etc) are set by default]
...
</br> ggplot(data = capacity_ae) + </br> 
</br> Or not: </br> 
</br> ggplot(capacity_ae) + </br> 
</br> (provided you have the arguments in the correct order!)
...
</br> Unspecified (yet required) arguments will often revert to default values
...
.green[x goes first and y second]
...
ggplot(data = capacity_ae) .blue[\+]
</br> .blue[geom_point](aes(x = dcubicles, y = dwait)) .blue[\+]
</br> .blue[geom_...] aes(x = dcubicles, y = dwait))
...
</br> So each point is given a colour which will depend on the value of staff_increase
...
The two sites in question have indeed seen an increase in staff levels which has had an effect on the dwait even though dcubicles are relatively low.
